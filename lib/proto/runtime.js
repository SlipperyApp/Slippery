...
const FRACLADDER=[[1.01,'1/100'],[1.02,'1/50'],[1.04,'1/25'],[1.05,'1/20'],[1.06,'1/16'],
 ...
 [1.73,'8/11'],[1.80,'4/5'],[1.83,'5/6'],[1.90,'9/10'],[1.91,'10/11'],[2.00,'1/1'],[2.10,'11/10'],
...
/* 08 · The challenge count was written twice — 3 beside the social button
   and 2 in the nav — so the product disagreed with itself on the same screen
   at desktop width, where both are visible at once. */
 ...
 * ═══════════════════════════════════════════════════════════════════════ */
/* 20 · ONE SOURCE FOR THE BOOKMAKER COLOURS.
   These were hex literals here, which meant the dots, the gallery tints and
   anything drawn later each carried their own copy of bet365's green. They
   are CSS custom properties now and this reads them, so a change lands
   everywhere at once — and a theme could override one if it ever needed to. */
const BOOK_TIER1={'bet365':'var(--bk-bet365)','Sky Bet':'var(--bk-skybet)',
 'Paddy Power':'var(--bk-paddypower)','Betfair':'var(--bk-betfair)',
 'Ladbrokes':'var(--bk-ladbrokes)','Coral':'var(--bk-coral)',
 'Betfred':'var(--bk-betfred)','William Hill':'var(--bk-williamhill)',
 'BoyleSports':'var(--bk-boylesports)','Unibet':'var(--bk-unibet)'};
const BOOK_TIER2=['var(--bk-h1)','var(--bk-h2)','var(--bk-h3)','var(--bk-h4)',
 'var(--bk-h5)','var(--bk-h6)','var(--bk-h7)','var(--bk-h8)','var(--bk-h9)'];
...
<header class="topbar">${BRANDMARK}
 <nav class="tnav" aria-label="Landing sections">${[['How it works','how'],['Pricing','price'],['FAQs','faq']].map(x=>`<button data-scroll="${x[1]}">${x[0]}</button>`).join('')}
 ...
 <div class="tgbox">
  <div class="tgcard"><span class="tgicon">${TG}</span>
   <div style="text-align:left;flex:1;min-width:0"><b style="font-size:14px;display:block">@SlipperyAppBot</b>
    <span style="font-size:11.5px;color:var(--t2)">Private chat or group</span></div>
...
<div class="lfoot"><div style="display:flex;justify-content:center">${BRAND}</div>
  <nav aria-label="Footer">${[['How it works','how'],['Pricing','price'],['Bookmakers','faq'],['FAQs','faq']].map(x=>`<button style="font-size:13px;color:var(--t2)" data-scroll="${x[1]}">${x[0]}</button>`).join('')}
...
function shotFrame(){return `<div class="shotframe" style="border-radius:14px;overflow:hidden;border:1px solid var(--line);background:var(--elev)">
 <div style="display:flex;align-items:center;gap:6px;padding:8px 12px;background:rgba(255,255,255,.05);border-bottom:1px solid var(--line)">
  ...
  <span class="mono" style="flex:1;text-align:center;font-size:10px;color:var(--t2)">slippery.app</span></div>
...
V.ledger={nav:'dash',tab:'LEDGER',run:1,html:()=>{
 /* THE FACET CHIPS LOOKED SELECTED AND FILTERED NOTHING. Clicking "Won 6"
    moved the highlight and left all twelve rows on screen — which is worse
    than having no filter at all, because the reader believes the list in
    front of them is six winning bets. `data-pickone` only ever swapped
    aria-current; nothing read it. */
 const c={};bets.forEach(b=>{c[b[0]]=(c[b[0]]||0)+1});
 const fsel=cur.ledgerFacet||'';
 const shown=fsel?bets.filter(b=>b[0]===fsel):bets;
 const staked=shown.reduce((t,b)=>t+b[5],0),ret=shown.reduce((t,b)=>t+b[6],0),net=ret-staked;
 /* Placed is its own outcome, so it needs its own chip — otherwise an
    each-way bet that placed is in neither Won nor Lost and the counts do not
    add up to the list. */
 const facets=[['All',bets.length,''],['Won',c.w||0,'w'],['Lost',c.l||0,'l']]
  .concat(c.p?[['Placed',c.p,'p']]:[]);
 return `<div class="pane">
 ...
 <div class="card" data-cardid="lrows">
  ...
  <div class="chips">${facets.map(f=>`<button class="chip ${f[2]}" aria-current="${fsel===f[2]}"
    data-facet="${f[2]}">${f[0]} ${f[1]}</button>`).join('')}</div>
  ${shown.slice(0,cur.ledgerShown||LEDGER_PAGE).map((b,i)=>betRow(b,bets.indexOf(b))).join('')}
  ${shown.length>(cur.ledgerShown||LEDGER_PAGE)
   ? `<div style="display:flex;flex-direction:column;align-items:center;gap:6px;padding-top:13px">
      ...
      <span class="lbl">Showing ${Math.min(cur.ledgerShown||LEDGER_PAGE,shown.length)} of ${shown.length}</span></div>`
   : `<div style="display:flex;justify-content:center;padding-top:13px">
      <span class="lbl">${fsel?shown.length+' of '+bets.length+' shown':'All '+bets.length+' shown'}</span></div>`}</div>
...
V.discover={narrow:1,nav:'soc',back:'social',title:'Discover groups',html:()=>`<div class="pane">
 ...
 <!-- 08 · Ordered by bets logged this month, then members. Ranking by size
      alone puts the biggest dead group first, and a dead group at the top
      kills the page for everybody who arrives after it. -->
 ${(() => {
   /* name, initials, members, units, roi, join mode, BETS THIS MONTH.
      Sorted on that last figure, then members. Weekend Multis is the case
      this exists for: 54 members and four bets all month, which would sit at
      the top of a list ranked by size and tell every arrival the product is
      empty. */
   const G=[['Ultras','U',12,'+41.2u','+6.1%','joined',218],
    ['HB Value','HB',38,'+22.8u','+3.4%','request',164],
    ['Irish Racing','IR',21,'+15.9u','+2.8%','code',97],
    ['Weekend Multis','WM',54,'\u22128.7u','\u22122.1%','open',4],
    ['Value Vault','VV',16,'+31.0u','+5.2%','request',73]];
   return [...G].sort((a,b)=>b[6]-a[6]||b[2]-a[2]);
 })().map(g=>{
  ...
  return `<div class="card t"><div class="hd" style="margin:0">
   <div style="display:flex;gap:10px;align-items:center"><span class="gpic" style="width:30px;height:30px;font-size:11px;border-radius:9px">${g[1]}</span>
    <div style="text-align:left"><b style="font-size:14px">${g[0]}</b>
     <div class="dd" style="font-size:11px;color:var(--t3)">${g[2]} Slippers \u00b7 ${g[6]} bets this month \u00b7 avg ROI ${g[4]}</div></div></div>
...
function feedRow(f){
 ...
 /* The reaction buttons carried data-react and nothing handled it, so all
    three were dead. The key is the row plus the glyph, because the feed has
    no ids of its own yet. */
 const me=who==='You';
 return `<div class="feedrow">
  ${me
   ? `<span class="feedwho">${avatarHTML(who,'user',26)}</span>`
   : `<button class="feedwho" data-personsel="${esc(who)}"
       aria-label="${esc(who)}'s profile">${avatarHTML(who,'user',26)}</button>`}
  <div class="feedbody">
  ...
  <div class="feedacts">${['🔥','👏','😬'].map(r=>{
    const key=who+'|'+what+'|'+r, mine=(cur.reacted||[]).includes(key);
    return `<button class="react${mine?' on':''}" data-react="${esc(key)}"
      aria-pressed="${mine}" aria-label="React ${r}">${r}</button>`}).join('')}</div>
 </div>`}
 ...
 ${people
  ...
  : `<div class="soclist">${GROUPROWS()}
      <div style="display:flex;gap:9px;margin-top:4px">
       <button class="btn ghost full sm" data-sheet="challenge">Challenges<span class="nbadge a" style="margin-left:7px">${CHALLENGE_COUNT}</span></button>
...
/* 05 · THE WIZARD DECLARED FIVE STEPS AND HAD TWO.
 *
 * Source and a review screen existed; dry run, resolve and commit did not,
 * and all four source buttons jumped straight past them to the same place.
 * So the strip along the top promised a process the product could not
 * perform — and step 4 pointed at "Fix problem bets", which was a sheet with
 * three dead rows and no screen behind it at all.
 *
 * NOTHING IS WRITTEN UNTIL COMMIT. That is the whole reason for a dry run,
 * and it is the promise the resolve step depends on: you cannot offer to fix
 * something that has already been saved.
 */
function wsteps(active){
 return `<ol class="wsteps" aria-label="Import progress">${IMPORT_STEPS.map((st,i)=>`
  <li class="${i===active?'on':i<active?'done':''}"${i===active?' aria-current="step"':''}>
   <b>${i+1} · ${esc(st[0])}</b><span>${esc(st[1])}</span></li>`).join('')}</ol>`}

/* The dry run's own figures. One place, so the four screens that quote them
   cannot disagree with each other. */
const DRY={read:312,importing:284,duplicates:19,decide:9};

V.impdry={narrow:1,nav:'imp',back:'imphist',title:'Dry run',html:()=>`<div class="pane">
 ${wsteps(2)}
 <div class="card"><div class="hd"><b>Nothing has been written</b>
  <span class="pill">CSV</span></div>
  ${[['Rows read',DRY.read,''],
     ['Will import',DRY.importing,'pos2'],
     ['Duplicates skipped',DRY.duplicates,''],
     ['Need a decision',DRY.decide,'warn']].map(r=>`
   <div class="setrow"><div class="k" style="font-size:13.5px">${r[0]}</div>
    <b class="mono ${r[2]}" style="font-size:13.5px">${r[1]}</b></div>`).join('')}
  <p class="note" style="margin-top:10px">Duplicates are matched on the bet, not the file —
   two screenshots of one slip are different files, and cropping one makes a third.</p></div>
 <div class="card t"><div class="lbl" style="margin-bottom:6px">What these will and will not touch</div>
  ${[['Net and turnover','g','Yes'],['The calendar','g','Yes'],
     ['Win rate, streaks, best and worst day','r','No']]
   .map(x=>`<div class="setrow" style="padding:8px 0"><div class="k" style="font-size:13px">${x[0]}</div>
    <span class="pill ${x[1]}">${x[2]}</span></div>`).join('')}</div>
 <button class="btn full" data-go="impfix">Resolve ${DRY.decide} · then import</button>
 <button class="btn ghost full sm" style="margin-top:9px" data-go="imphist">Start again</button></div>`};

/* 05 · RESOLVE. `Fix problem bets` was a sheet with three `data-toast` rows
   and a note promising "Each one previews before it changes anything" — a
   preview that did not exist. It is a screen now, it is the wizard's step 4,
   and the gallery's FIX tile has somewhere to land. */
const FIXQUEUE=[
 ['Combined selections',4,'These look like several bets joined into one line. Splitting them is the only way the legs can grade separately.'],
 ['No fixture matched',2,'We could not find the match, so these cannot settle themselves. Naming the fixture fixes that.'],
 ['No sport',1,'Without a sport they are missing from every breakdown, though they still count toward net.']];

V.impfix={narrow:1,nav:'imp',back:'impdry',title:'Fix problem bets',html:()=>`<div class="pane">
 ${wsteps(3)}
 <div class="card"><div class="hd"><b>${FIXQUEUE.reduce((t,f)=>t+f[1],0)} need a decision</b>
  <span class="pill a">Before import</span></div>
  ${FIXQUEUE.map(f=>`<button class="setrow" data-sheet="fixone">
   <div><div class="k">${esc(f[0])}</div><div class="dd">${esc(f[2])}</div></div>
   <span class="pill a">${f[1]}</span></button>`).join('')}
  <p class="note" style="margin-top:10px">Each one previews before it changes anything,
   and nothing here has been saved yet.</p></div>
 <button class="btn full" data-go="impcommit">Import ${DRY.importing} rows</button>
 <button class="btn ghost full sm" style="margin-top:9px" data-go="impcommit">Skip these and import ${DRY.importing - DRY.decide}</button></div>`};

/* 05 · COMMIT, with the undo window the step name has always promised. */
V.impcommit={narrow:1,nav:'imp',back:'impfix',title:'Imported',html:()=>`<div class="pane">
 ${wsteps(4)}
 <div class="card" style="text-align:center;border-color:var(--pos)">
  <div class="lbl">Imported</div>
  <div class="big mono pos2" style="margin:4px 0 2px">${DRY.importing}</div>
  <div class="dd">rows, ${DRY.duplicates} duplicates skipped</div>
  <p class="note" style="margin:11px 0 0">They are in your imported history, kept apart from
   your slip-backed record. Net and the calendar move; win rate, streaks and best
   and worst day do not.</p></div>
 <button class="btn full" data-go="history">See imported history</button>
 <button class="btn ghost full sm" style="margin-top:9px" data-toast="Import undone">Undo this import</button>
 <p class="note" style="text-align:center;margin-top:10px">Undo is available for 24 hours.</p></div>`};
...
V.imphistreview={narrow:1,nav:'imp',back:'imphist',title:'Check what was read',html:()=>`<div class="pane">
 ${wsteps(1)}
 ...
 <!-- 05 · On to the dry run rather than straight into the ledger. Nothing is
      written until commit, which is the entire point of the three steps that
      used to be missing. -->
 <button class="btn full" data-go="impdry">Continue to the dry run</button></div>`};
...
V.su4={bare:1,html:()=>wiz(4,6,'What is one unit?','Your normal stake. It lets you compare with people who bet bigger or smaller.',`
 ${(() => {
   const preset=['£5','£10','£20','£25','£50','£100'];
   /* "Custom" highlighted itself and then offered nothing to type into, so
      anybody whose unit is not one of the six had no way to say so. It
      reveals a field, and the field is what sets the unit. */
   const isCustom=cur.unitCustom||!preset.includes('£'+cur.unit);
   return `<div class="chips" style="margin-top:10px">${
    preset.concat('Custom').map(x=>`<button class="chip" aria-current="${
      x==='Custom'?isCustom:(!isCustom&&x==='£'+cur.unit)}" data-setunit="${x}">${x}</button>`).join('')}</div>
   ${isCustom?`<label class="flabel" for="customunit" style="margin-top:11px">Your unit</label>
    <input class="field mono" id="customunit" data-customunit inputmode="decimal"
      autocomplete="off" value="${cur.unit}" aria-label="Your unit in pounds">`:''}`;
 })()}
...
Object.assign(SH,{
 ...
 challenge:()=>{const g=cur.chalGroup||'All';
  ...
  <!-- 08 · The tab used to be switched by matching the button's own text
       against /Hall of fame|Running/, because segHTML was called without an
       attr callback. Any copy change silently broke the control. -->
  <div style="max-width:340px;margin-bottom:12px">${segHTML(['Running','Hall of fame'],
    cur.chalTab||'Running',x=>`data-chaltab="${esc(x)}"`)}</div>
 ...
 /* 05 · SLIP LEFT, PARSED BET RIGHT. This was one column showing a redrawn
    slip and nothing to compare it against — but the pairing IS the trust
    proposition: what the bookmaker printed, beside what Slippery read from
    it, so anybody can check the reader in a second rather than taking its
    word. On a phone the two stack, slip first. */
 slipimg:()=>`<div class="hd"><b style="font-size:16px">Original slip</b><span class="pill">19 Aug 19:12</span></div>
  <div class="lbox">
   <div class="lbside">
    <div class="lbl" style="margin-bottom:6px">As received</div>
    <div class="card t" style="background:var(--bg);margin:0">
     ...
     <div class="slipfoot" style="font-size:11px"><div>Stake<b class="mono" style="font-size:13px">£100.00</b></div>
      <div style="text-align:right">Returns<b class="mono" style="font-size:13px">£180.00</b></div></div></div></div></div>
   <div class="lbside">
    <div class="lbl" style="margin-bottom:6px">What Slippery read</div>
    <div class="card t" style="margin:0">
     ${[['Bookmaker','bet365'],['Bet type','4-leg multiple'],['Stake',amount(100)],
        ['Combined price','1.80'],['Returns',amount(180)],['Placed','19 Aug 2026, 19:12'],
        ['Every leg','Read'],['Result','Won']].map(r=>`
      <div class="setrow" style="padding:7px 0"><div class="k" style="font-size:13px">${r[0]}</div>
       <b class="mono" style="font-size:13px">${esc(r[1])}</b></div>`).join('')}
     <p class="note" style="margin:8px 0 0">Anything read wrong is one tap to correct, and
      the correction teaches the reader for that bookmaker's layout.</p></div></div></div>
  ...
  <div style="display:flex;gap:9px"><button class="btn ghost full sm" data-toast="Downloaded">Download image</button>
   <button class="btn ghost full sm" data-sheet="wrong">Something is wrong</button></div>`,
 ...
 /* Defined twice, and the second silently won — so the better copy, which
    names the group you are adding to, was dead. One definition, and it is
    the one that says "Add a market to Handicaps" rather than "Add a market". */
 ...
 /* 05 · THE PREVIEW THE NOTE ALWAYS PROMISED. "Each one previews before it
    changes anything" sat under three rows that raised a toast and previewed
    nothing. Splitting a combined selection is destructive-looking and
    reversible, so it has to show both sides before it does it. */
 fixone:()=>`<div class="hd"><b style="font-size:16px">Combined selections</b>
   <span class="pill a">4 bets</span></div>
  <p class="note" style="margin:0 0 12px">These look like several bets joined into one
   line. Split them and each leg can grade on its own; leave them and the whole
   row settles as one.</p>
  <div class="card t"><div class="lbl" style="margin-bottom:6px">As read</div>
   <div class="k" style="font-size:13px">Arsenal to win &amp; Over 2.5 Goals &amp; Sinner to win</div>
   <div class="dd">£25.00 · 6.84 · bet365</div></div>
  <div class="card t" style="border-color:var(--pos)"><div class="lbl" style="margin-bottom:6px">After splitting</div>
   ${[['Arsenal to win','1.90'],['Over 2.5 Goals','1.80'],['Sinner to win','2.00']].map(l=>`
    <div class="setrow" style="padding:7px 0"><div class="k" style="font-size:13px">${l[0]}</div>
     <b class="mono" style="font-size:13px">${l[1]}</b></div>`).join('')}
   <p class="note" style="margin:8px 0 0">One bet, three legs. Same stake, same combined price.</p></div>
  <button class="btn full" style="margin-top:12px" data-toast="4 bets split">Split all 4</button>
  <button class="btn ghost full sm" style="margin-top:9px" data-close>Leave them as they are</button>`,
 fix:()=>`<div class="hd"><b style="font-size:16px">Fix problem bets</b><span class="pill a">7</span></div>
  ${[['Combined selections','4 bets look like several joined together'],['No fixture matched','2 bets'],['No sport','1 bet']].map(o=>`
   <button class="setrow" data-sheet="fixone"><div><div class="k">${o[0]}</div><div class="dd">${o[1]}</div></div><span class="caret" style="transform:rotate(-90deg)">▾</span></button>`).join('')}
  <p class="note">Each one previews before it changes anything.</p>
  <button class="btn ghost full sm" style="margin-top:11px" data-go="impfix">Open the full queue</button>`,
...
const VIEWTITLE={
 ...
 impdry:'Dry run', impfix:'Fix problem bets', impcommit:'Imported',
...
const NAVI=a=>`<nav class="navbar" aria-label="Sections">
 ...
 <div class="navextra">
  ...
  <button class="navsub" data-sheet="challenge">Challenges<span class="nbadge a">${CHALLENGE_COUNT}</span></button>
...
const SHEET_SIZE={
 /* 05 · The slip and the parsed bet side by side need the content width, not
    the form width — at 560 they stack on a desktop and the comparison, which
    is the entire point, is lost. */
...
function sheet(k){if(!SH[k])return;
 ...
 ph.insertAdjacentHTML('beforeend',`<div class="scrim" data-close></div><div class="sheet" data-sheetsize="${sheetSize(k)}" role="dialog" aria-modal="true" aria-label="${k}"><div class="grab"></div>${SH[k]()}</div>`);
 lockBg(true);
...
doc.addEventListener('click',e=>{
 ...
 /* `data-size` is the module-size picker, whose value is "net:w". The sheet
    element briefly carried `data-size="wide"` for its width, so EVERY click
    inside EVERY sheet matched here, split a value with no colon, and threw on
    an undefined object — aborting the handler before any real branch ran. The
    sheet uses `data-sheetsize` now; this one is unambiguous again. */
 const sz=e.target.closest('[data-size]');if(sz){const [k,z]=sz.dataset.size.split(':');
  if(!z){return}
  /* And it wrote `cur.size` while szCard reads `cur.SIZE` — a case mismatch,
     so the picker had never changed a module's size even when it did run. */
  cur.SIZE[k]=z;
 ...
 const su=e.target.closest('[data-setunit]');if(su){const v=su.dataset.setunit;
  cur.unitCustom=(v==='Custom');
  if(v!=='Custom')cur.unit=parseInt(v.replace('£',''),10);
  /* Repaint so the field appears or disappears with the choice. */
  if(v==='Custom'){repaint();setTimeout(()=>{
    const f=ph.querySelector('[data-customunit]');if(f){f.focus();f.select()}},60);return}
 ...
 /* Reactions toggle and persist for the session; the key is row + glyph
    because the feed rows have no ids of their own yet. */
 const rxb=e.target.closest('[data-react]');
 if(rxb){const k=rxb.dataset.react;const l=cur.reacted||(cur.reacted=[]);
  const i=l.indexOf(k);i<0?l.push(k):l.splice(i,1);repaint();return}
 ...
 /* 08 · Its own branch, early, like every other action. The old code lived in
    a late `.seg button` block and matched on the button's TEXT, so an earlier
    branch could swallow it and a copy change would break it silently. */
 const ct=e.target.closest('[data-chaltab]');
 if(ct){cur.chalTab=ct.dataset.chaltab;reSheet('challenge');return}
 ...
 {/* 08 · The challenge tab is handled early now, by data-chaltab. */}
 /* Clicking the chip that is already on clears the filter, which is what
    every filter chip in every other product does. */
 const fc=e.target.closest('[data-facet]');
 if(fc){const v=fc.dataset.facet;
  cur.ledgerFacet=(cur.ledgerFacet===v||v==='')?'':v;
  cur.ledgerShown=null;repaint();return}

 /* The signup plan cells moved their highlight and recorded nothing, so the
    choice was lost the moment the screen repainted and checkout had no plan
    to read. `data-pickone` only ever swapped aria-current. */
 const pl=e.target.closest('[data-plan]');
 if(pl){cur.plan=pl.dataset.plan;
  [...pl.parentElement.querySelectorAll('[data-plan]')].forEach(x=>{
   const on=x===pl;x.setAttribute('aria-current',on);x.classList.toggle('on',on)});
  return}
...
doc.addEventListener('input',e=>{
 ...
 /* A custom unit. Updated on every keystroke but NOT repainted, or the field
    being typed into would be destroyed and take the caret with it — the two
    worked examples below it are patched in place instead. */
 const cu=e.target.closest('[data-customunit]');
 if(cu){const v=parseFloat(String(cu.value).replace(/[^0-9.]/g,''));
  if(v>0){cur.unit=v;
   const eg=document.getElementById('unitEg');if(eg)eg.innerHTML=unitExample(cur.unit);
   const pv=document.getElementById('unitPrev');if(pv)pv.innerHTML=unitPreview(cur.unit);}
  return}
 ...
 *
 * The fill stops at 28%. Past that the cell reaches a mid luminance that no
 * text colour clears 4.5:1 against, which is worked through at the ramp
 * itself below.
 *
...
function calCell(d,today){
 ...
 /* THE RAMP JUMPS THE BAND THAT CARRIES NO TEXT.
  *
  * Between 0.46 and 0.64 a cell is mid-luminance and nothing clears 4.5:1
  * against it — not --t1, not --t2, not solid --bg. The old ramp ran 0.14 to
  * 0.76 straight through it, and `heavy` switched at 0.42, inside the band,
  * so both branches failed: today measured 1.67:1, a light cell 2.4:1.
  *
  * Capping the ramp below the band passes the audit and costs the module its
  * point — +£264 and +£64 come out the same green, which is the exact
  * complaint the intensity fill exists to answer. So it jumps instead. Two
  * bands, and the gap between them is the dead zone:
  *
  *   light  0.10-0.28  the figure keeps its semantic colour, date --t1
  *   strong 0.70-0.84  figure and date go solid --bg
  *
  * Worst case across all eight themes is 4.71:1, the loss figure in liquid.
  * Moving either band without re-running that arithmetic puts text back on a
  * ground that cannot carry it. */
 const n=v?Math.min(1,Math.abs(v)/max):0;
 const a=flat?0.16:(v?(n<0.5?0.10+n*0.36:0.70+(n-0.5)*0.28):0);
 const heavy=!flat&&a>=0.70;
