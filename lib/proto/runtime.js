...
function calCell(d,today){
 ...
 /* THE RAMP STOPS SHORT OF THE BAND THAT CARRIES NO TEXT.
  *
  * It used to run 0.14 to 0.76 and pass straight through 0.46-0.64, where the
  * cell is mid-luminance and nothing clears 4.5:1 against it — not --t1, not
  * --t2, and not solid --bg. `heavy` was meant to rescue the top by flipping
  * the text dark, but it switched at 0.42, inside the dead band, so both
  * branches failed: measured 1.67:1 on today and 2.4:1 on a light cell.
  *
  * 0.10 to 0.28 is the widest ramp where the figure keeps its semantic colour
  * and still reads. Checked against all eight themes: the worst case is the
  * loss figure in liquid at 4.71:1. Widening this without re-running that
  * arithmetic will silently reintroduce the dead band. */
 const a=flat?0.16:(v?Math.min(1,Math.abs(v)/max)*0.18+0.10:0);
 ...
 const fig=cur.showIn==='Units'?(v>0?'+':'−')+Math.abs(v/cur.unit).toFixed(1)+'u'
   :(v>0?'+':'−')+Math.abs(v);
 return `<${v?'button':'div'} class="c ${v>0?'p':v<0?'n':''} ${none?'none':''} ${fut?'fut':''} ${d===today?'today':''}"
