...
const DEFAULT_ABOVE=['net','cal','running','recent','alltime','oddsband','curve','stake','market','sport','tips','book','clv'];
...
/* 05 · THE ORDER IS THE PACKING. Each row of the twelve-column grid adds to
   exactly twelve, so no row ends with a ragged gap:
     net 12 · cal 5 + running 7 · recent 7 (two rows) + alltime 5, oddsband 5
     · curve 12 · stake 6 + market 6 · sport 4 + tips 4 + book 4 · clv 12
   Change this list and you change the packing, so check the arithmetic. */
const DEFAULT_ORDER=['net','cal','running','recent','alltime','oddsband','curve','stake',
 'market','sport','tips','book','clv'];
...
function perfCard(){
 ...
 const W=innerWidth>=1000?1120:Math.max(340,Math.round(innerWidth-64));
...
<div class="lsec c" data-sec="settle"><h2 class="lh2">Every slip, settled either way.</h2>
 ${film('settling','How Slippery settles a bet')}
 <!-- The film's first frame lists four; the product has six. Naming them in
      text means the heading is not making a promise the still does not keep,
      and it is the accessible version of the same information. -->
...
function unitPreview(u){
 ...
 return `<div class="unitprev">
  <div class="hd" style="margin-bottom:8px"><b style="font-size:13px">With a £${u} unit</b>
  ...
  ${rows.map(([n,p,w])=>{const ret=w?u*p-u:-u;
   return `<div class="uprow"><span class="uo ${w?'w':'l'}">${w?'✓':'✕'}</span>
    ...
    <span class="us">£${u.toFixed(2)}</span>
    <b class="uv ${w?'pos2':'neg2'}">${ret>0?'+':'−'}£${Math.abs(ret).toFixed(2)}</b></div>`}).join('')}
  <div class="upfoot"><span>Net from these four</span>
   <b class="${total>0?'pos2':'neg2'}">${total>0?'+':'−'}£${Math.abs(total).toFixed(2)}</b></div>
 </div>`}
...
function GROUPROWS(){return GROUPLIST.map(g=>{
 ...
 return `<button class="grow2" data-go="groupdetail">
...
function PEOPLEROWS(){return PEOPLELIST.map((p,i)=>`
 <button class="prow" data-go="person">
...
V.social={nav:'soc',tab:'GROUPS',html:()=>{
 ...
 <div class="hd"><span class="lbl ${people?'pe':'gr'}">${people?'Connected':'Your groups'}</span>
  <button class="chip" data-sheet="${people?'tipsters':'creategroup'}">${people?'Find people':'+ New group'}</button></div>
 ${people
...
const BAR=v=>v.back?`<div class="topback"><button data-go="${v.back}" style="font-size:20px">‹</button>${v.title}</div>`
 :`<div class="appbar">${BRAND}
   <div class="barright">
    ${v.run&&BR_OPEN>0?`<button class="runpill" data-sheet="exposure" aria-label="Open exposure, £${BR_OPEN} at risk, ${exposurePct().toFixed(1)} percent of bankroll"><span class="dot"></span><span class="rplab">Open exposure</span><span class="mono">£${BR_OPEN}</span><span class="pct">${exposurePct().toFixed(1)}%</span></button>`:''}
