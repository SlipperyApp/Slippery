...
const cur={netOrder:null,chalGroup:'All',chalTab:'Running',shareKinds:['Summary'],netShow:['bets','units'],calUnits:false,promo:'SEUN-8QK4',unit:25,oddsFmt:'Decimal',showIn:'Currency',view:'landing',theme:'carbon',per:'M',calDates:true,weekStart:1,demoSeen:false,signedIn:false,
 ...
 SIZE:{net:'w',cal:'f',curve:'w',recent:'w',alltime:'c',clv:'c',months:'w',
  stake:'s',book:'c',market:'s',tips:'c',sport:'c',course:'s',odds:'s',dow:'s'},
...
function breakdown(id,title,rows){return `<div class="card" data-cardid="${id}"><div class="hd"><b>${title}</b><span class="lbl">${PERIODS[cur.per].lab}</span></div>
 <div class="grow">${rows.map(r=>`<div class="barline"><div class="h"><span>${r[0]}</span><span class="hv">${sparkline(r[4],r[3])}<b class="mono" style="color:var(--${r[3]?'neg':'pos'})">${r[1]}</b></span></div><div class="track"><div class="fill ${r[3]?'neg':''}" data-w="${r[2]}"></div></div></div>`).join('')}</div></div>`}
...
function stakeChart(rows,unit){
 const over=rows.map(r=>Math.max(0,r[1]-unit));
 const worst=Math.max(unit*0.9,...over);
 return `<div class="stakeplot">
  <div class="stakescale"><span>on plan · £${unit}</span><span>+£${worst.toFixed(0)} over</span></div>
  ${rows.map(r=>{
   const ov=Math.max(0,r[1]-unit), pct=Math.min(100,(ov/worst)*100);
   const tone=ov>unit*0.5?'neg':ov>unit*0.15?'a':'p';
   const on=ov<=unit*0.05;
   return `<div class="stakerow">
    <span class="sw2">${r[0]}</span>
    <span class="strack">
     <i class="sbase"></i>
     ${on?'<i class="sdot"></i>'
        :`<i class="sbar ${tone}" data-bar style="width:0;--tw:${pct.toFixed(1)}%"></i>`}
    </span>
    <b class="sval mono ${on?'':tone}">${on?'on plan':'+£'+ov.toFixed(0)}</b>
   </div>`}).join('')}
 </div>`}
...
const CARDFN={net:netCard,cal:calCard,recent:recentCard,
 ...
 sport:()=>bdCard('sport','By sport',[['Football','£812',86,0,[110,190,260,340,468,590,704,812]],['Horse racing','£262',31,0,[44,70,120,98,166,198,240,262]],['Tennis','−£58',9,1,[-5,-12,-9,-22,-30,-41,-50,-58]]]),
 book:()=>breakdown('book','By bookmaker',[['bet365','£742',88,0,[120,180,150,240,310,402,588,742]],['Sky Bet','£310',42,0,[40,96,88,140,190,214,268,310]],['Ladbrokes','−£96',14,1,[-8,-14,-30,-22,-51,-64,-88,-96]]]),
 market:()=>breakdown('market','By market',[['Home win','£412',72,0,[60,90,145,132,208,296,350,412]],['Both teams to score','£268',47,0,[30,58,96,120,164,190,236,268]],['Overs','£141',25,0,[18,44,38,72,90,116,128,141]],['Player to score','£96',17,0,[12,20,44,36,58,71,88,96]],['Unders','−£62',11,1,[-6,-10,-24,-18,-33,-44,-55,-62]]]),
 tips:()=>breakdown('tips','By tipster',[['Own picks','£552',82,0,[80,140,190,265,330,418,486,552]],['BlueSlip','£404',62,0,[50,88,132,176,240,300,356,404]]]),
 alltime:alltimeCard};
...
<div class="lsec c" data-sec="settle"><h2 class="lh2">Every slip, settled either way.</h2>
 ${film('settling','How Slippery settles a bet')}</div>

<div class="lsec c" data-sec="tour"><h2 class="lh2">See it in action.</h2>
 ${film('in-action','A tour of Slippery')}</div>
...
<div class="lsec c"><h2 class="lh2">Send a bet on Telegram.<br>It is tracked instantly.</h2>
 <div class="tgbox">
  ...
  ${film('bot','How the Telegram bot works')}</div></div>
...
V.auth={bare:1,html:()=>`<div class="pane authpane">
 ...
 <input class="field" id="authEmail" type="email" inputmode="email" autocomplete="email"
   spellcheck="false" placeholder="you@example.com" data-authemail>
 ...
 <p class="note authneed" id="docHint" aria-live="polite"></p>
...
async function authContinue(button){
 const em=ph.querySelector('[data-authemail]'),pw=ph.querySelector('[data-authpass]');
 if(!em||!pw)return;
 button.disabled=true;const was=button.textContent;button.textContent='Checking…';
 let r;
 try{
 ...
 button.textContent=was;button.disabled=false;checkDocs();
