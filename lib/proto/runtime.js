...
/* THE STORE BADGES.
 *
 * Drawn to specification and to be replaced with the official downloaded
 * assets before launch: Apple and Google both forbid recolouring or
 * modification, both require clear space of a quarter the badge height,
 * Apple requires forty pixels minimum onscreen, and neither permits a
 * modified "coming soon" badge, which is why that line is separate text.
 *
 * The optimisation here is the lettering. SVG <text> in Helvetica resolves to
 * a different face on every platform: Helvetica on a Mac, Arial on Windows,
 * Roboto or Liberation Sans on Android and Linux. Each has different advance
 * widths, so the words drifted inside the black box and on a narrow face they
 * sat short of the edge while on a wide one they ran past it.
 *
 * `textLength` with `lengthAdjust="spacingAndGlyphs"` fixes the box the words
 * must occupy, and the renderer fits whatever face it resolved into exactly
 * that width. The badge is then metrically identical everywhere, which is the
 * whole point of a badge, without shipping a font for two lines of text. */
...
<text x="41" y="14.5" fill="#fff" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif" font-size="8" textLength="43" lengthAdjust="spacingAndGlyphs">Download on the</text>
<text x="41" y="30" fill="#fff" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif" font-size="16.5" font-weight="500" textLength="70" lengthAdjust="spacingAndGlyphs">App Store</text></svg></span>`;
...
<text x="47" y="15" fill="#fff" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif" font-size="7.5" textLength="40" lengthAdjust="spacingAndGlyphs">GET IT ON</text>
<text x="46" y="31" fill="#fff" font-family="-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif" font-size="16.5" font-weight="500" textLength="94" lengthAdjust="spacingAndGlyphs">Google Play</text></svg></span>`;
const BRAND=`<button class="brand" data-home aria-label="Slippery, home">${MARK}<span aria-hidden="true">Slipp<em>ery</em></span></button>`;
...
/* Offered at signup, grouped by platform because brands on one platform
   share a slip layout and therefore read the same way. This list is the
   prototype's, exactly.
   WORTH THE OWNER'S ATTENTION: bet365 is not on it, and bet365 is the only
   bookmaker in the product that grades Asian handicaps, where a whole line
   pushes rather than losing. Anyone who bets with it has to reach it through
   "Add a custom bookmaker", and the grader knows the brand either way. Say
   the word and it goes in the Other group. */
...
V.ledger={nav:'dash',tab:'LEDGER',run:1,html:()=>{
 /* EVERY COUNT FROM ONE PASS OVER ONE SET, AND NO EMPTY FACETS.
    Cashed and Void were hardcoded 2 and 1 beside an All that counted the
    rows, so the strip read 6 + 6 + 2 + 1 against a total of 12 and could
    never add up. That is the defect this product is not allowed to have: a
    banner saying 486, a ledger listing 482 and facets summing to 474.
    Every count now comes from the same pass, and a facet with nothing in it
    is not drawn, so the facets always total the rows. */
 ...
 * phone, which is how the last card came to sit six pixels underneath it.
 * One ResizeObserver, one custom property, and the two cannot disagree. */
let navRO=null;
function measureNav(){
 ...
 const set=()=>{
  ...
  if(h)ph.style.setProperty('--navh',h+'px');
 };
 set();
 if(navRO)navRO.disconnect();
 /* Re-measured rather than assumed: rotating the phone, showing the keyboard
    and changing the text size all move it. */
 ...
 * disappear again when you scroll back up. `.offscreen` is the opposite, and
 * it toggles both ways, because the point of it is to stop paying for motion
 * that nobody can see. One observer answers both, with a margin so a section
 * is already moving by the time it comes into view rather than starting the
 * moment its first pixel lands. */
function revealOn(){const root=ph.querySelector('.body');if(!root)return;
 ...
 /* ROOT IS THE VIEWPORT, NOT THE SCROLLER.
    Below 1000px `.body` is the scroll container; at and above it the body
    becomes `overflow:visible` and the document scrolls instead. An observer
    rooted on an element that is not a scroll container never fires, which is
    why every section stayed animating on a laptop while the phone was fine.
    The viewport works for both: intersection is computed through the
    clipping ancestors, so content scrolled out of `.body` is correctly
    reported as out of view either way. */
 ...
 els.forEach((el,i)=>{
  ...
  /* The first two are above the fold on every width, so they are never
     hidden and never wait for the observer to say so. */
  if(i<2)el.classList.add('in');else el.classList.add('offscreen');
  revIO.observe(el);
 })}

/* A locked phone or a background tab should be doing nothing at all. This is
   free and it is the single biggest saving on a page left open. */
