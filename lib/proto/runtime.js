...
<svg viewBox="0 0 140 44" role="img"><rect x=".75" y=".75" width="138.5" height="42.5" rx="9" fill="none" stroke="currentColor" stroke-opacity=".38" stroke-width="1.5"/>
...
<svg viewBox="0 0 152 44" role="img"><rect x=".75" y=".75" width="150.5" height="42.5" rx="9" fill="none" stroke="currentColor" stroke-opacity=".38" stroke-width="1.5"/>
...
const BRAND=`<button class="brand" data-go="landing" data-home>${WORDMARK}</button>`;
const BRANDMARK=`<button class="brand" data-go="landing" data-home>${MARK}${WORDMARK}</button>`;
...
const bets=[
 ...
 ['w','York 16:10','Selection 9','Win',2.35,25,58.75,'+£33.75','+1.35u','Sky Bet','Own picks','Screenshot','16 Aug 2026, 16:10',0,null],
...
const BR_START=1000, BR_OPEN=88;
const bankroll=()=>BR_START+((cur.adaptBr!==false&&typeof PERIODS!=='undefined'&&PERIODS.All)?PERIODS.All.net:0);
const exposurePct=()=>(BR_OPEN/bankroll()*100);
...
const betRow=(b,i)=>`<button class="bet" data-bet="${i}">${cur.bulk?`<span style="width:18px;height:18px;border-radius:5px;flex:0 0 auto;margin-top:3px;border:1.5px solid ${i<3?'var(--pos)':'var(--line)'};background:${i<3?'color-mix(in srgb,var(--pos) 22%,transparent)':'none'};display:grid;place-items:center;font-size:10px;color:var(--pos)">${i<3?'✓':''}</span>`:`<div class="o ${b[0]}">${b[0]==='w'?'✓':'✕'}</div>`}
 <div class="m"><div class="n">${b[1]}${b[13]?'<span class="tag">IMPORTED</span>':''}</div>
  <div class="d">${b[2]} · ${fmtOdds(b[4])} · £${b[5].toFixed(2)} · ${b[9]}</div></div>
 ${showVal(b[7],b[8],b[0]==='w'?'pos':'neg')}</button>`;
...
const CARDS=[['net','Net and target'],['cal','Calendar'],['recent','Recent bets'],['alltime','All time'],
 ['book','By bookmaker'],['market','By market'],['tips','By tipster'],['sport','By sport'],['curve','Profit over time'],['months','Month by month'],['stake','Staking discipline'],['clv','Closing line value']];
const unitExample=u=>`<div class="row" style="font-size:12.5px;justify-content:space-between">
  <span>Stake £${u}, returns £${u*2} <span style="color:var(--t4)">so profit is £${u}</span></span><b class="mono" style="color:var(--pos)">+1.00u</b></div>
 <div class="row" style="font-size:12.5px;justify-content:space-between;margin-top:7px">
  <span>Stake £${u*2}, returns £0 <span style="color:var(--t4)">so loss is £${u*2}</span></span><b class="mono" style="color:var(--neg)">−2.00u</b></div>
 <p class="note" style="margin-top:8px">Units track profit, not what comes back.</p>`;
const DEFAULT_ABOVE=['net','cal','recent','alltime','months','stake','book','market','tips','sport','clv'];
const DEFAULT_ORDER=['net','cal','recent','alltime','months','stake','book','market','tips','sport','clv','curve'];
...
const cur={netOrder:null,chalGroup:'All',chalTab:'Running',shareKinds:['Summary'],netShow:['bets','units'],calUnits:false,promo:'SEUN-8QK4',unit:25,oddsFmt:'Decimal',showIn:'Currency',view:'landing',theme:'carbon',per:'M',calDates:true,weekStart:1,demoSeen:false,signedIn:false,
 running:4,risk:88,above:['net','cal','recent','alltime','months','stake','book','market','tips','sport','clv'],
 /* canonical size per widget by content shape, assigned by the system */
 SIZE:{net:'w',cal:'t',curve:'s',recent:'t',alltime:'c',clv:'c',months:'s',stake:'s',book:'s',market:'s',tips:'c',sport:'s'},
 LOCKED:['net','cal','recent'],order:['net','cal','recent','alltime','months','stake','book','market','tips','sport','clv','curve']};
const V={},SH={};
...
function netCard(){
 ...
 <div class="row">${NETFIGS().map(k=>({bets:`Bets <b>${d.bets}</b>`,
   ...
   turnover:`Turnover <b>£${(d.to-d.void).toLocaleString('en-GB')}</b>`,
   roi:`ROI <b class="mono">+${(d.net/(d.to-d.void)*100).toFixed(1)}%</b>`,
...
function calCard(){return `<div class="card" data-cardid="cal"><div class="hd"><b>August 2026</b>
  <span style="display:flex;gap:6px;align-items:center">
   <button class="iconb" data-sheet="calopts" aria-label="Calendar options">⋯</button>
   <button class="chip mobonly2" data-calexpand>${cur.calOpen?'Collapse':'Expand'}</button></span></div>
 <div class="calwrap ${cur.calOpen?'open':''}" data-calwrap><div class="cal chartfill" data-cal="${cur.calOpen?'month':'auto'}"></div></div>
...
function szCard(k){const z=cur.SIZE[k]||'s';
 return CARDFN[k]().replace('class="card"',`class="card sz-${z}"`)}
function recentCard(){return `<div class="card" data-cardid="recent"><div class="hd"><b>Recent bets</b><button class="chip" data-go="ledger">View all</button></div>
 <div class="scrolly">${bets.slice(0,8).map(betRow).join('')}</div>
 <button class="btn ghost full sm" style="margin-top:10px;flex:0 0 auto" data-go="ledger">See all ${bets.length} in the ledger</button></div>`}
function breakdown(id,title,rows){return `<div class="card" data-cardid="${id}"><div class="hd"><b>${title}</b><span class="lbl">${PERIODS[cur.per].lab}</span></div>
 <div class="grow">${rows.map(r=>`<div class="barline"><div class="h"><span>${r[0]}</span><b class="mono" style="color:var(--${r[3]?'neg':'pos'})">${r[1]}</b></div><div class="track"><div class="fill ${r[3]?'neg':''}" data-w="${r[2]}"></div></div></div>`).join('')}</div></div>`}
...
function stakeChart(rows,unit){const mx=Math.max(unit*1.9,...rows.map(r=>r[1]));
 ...
 return `<div style="position:relative;height:${H}px;min-height:${H}px;flex:0 0 auto;margin:10px 0 6px;padding-right:78px">
  <div style="position:absolute;left:0;right:78px;top:${y(unit).toFixed(1)}px;border-top:2px dashed var(--s);opacity:.85"></div>
  <span style="position:absolute;right:0;top:${(y(unit)-7).toFixed(1)}px;font-size:10px;color:var(--s);font-weight:600;white-space:nowrap;letter-spacing:.01em">1 unit · £${unit}</span>
  <div style="display:flex;align-items:flex-end;gap:7px;height:100%;position:absolute;left:0;right:78px;top:0;bottom:0">
   ${rows.map(r=>{const hot=r[1]>unit,hh=(r[1]/mx)*H;
    return `<div data-bar title="${r[0]}: £${r[1]}" style="flex:1;height:0;--th:${hh.toFixed(1)}px;border-radius:4px 4px 2px 2px;
     background:${r[1]>unit*1.5?'var(--neg)':r[1]>unit*1.15?'var(--a)':'var(--p)'};opacity:${r[1]>unit*1.15?.95:.8}"></div>`}).join('')}</div></div>
 <div style="display:flex;gap:7px;padding-right:78px">${rows.map((r,i)=>`<span style="flex:1;text-align:center;font-size:9.5px;color:var(--t4)">${i%2?'':r[0]}</span>`).join('')}</div>`}
...
const CARDFN={net:netCard,cal:calCard,recent:recentCard,
 curve:()=>`<div class="card" data-cardid="curve"><div class="hd"><b>Profit over time</b><span class="lbl">${PERIODS[cur.per].lab}</span></div>
  <div class="chartwrap">${lineChart(CURVE,'a')}</div>
 ...
 clv:()=>`<div class="card" data-cardid="clv"><div class="hd"><b>Closing line value</b>
  <span class="lbl">${PERIODS[cur.per].lab}</span></div>
  <div class="grow"><div class="mono" style="font-size:30px;font-weight:700;letter-spacing:-.03em;color:var(--pos)">▲ +2.8%</div>
   <p class="note" style="margin:7px 0 0">You are beating the closing price on average. It predicts long-run profit better than win rate.</p></div>
  <div class="row" style="font-size:11.5px;flex:0 0 auto"><span>Beat close <b>58%</b></span><span>Bets priced <b>84</b></span></div></div>`,
 stake:()=>`<div class="card" data-cardid="stake"><div class="hd"><b>Staking discipline</b><span class="lbl">Last 6 weeks</span></div>
  ${stakeChart([['14 Jul',25],['21 Jul',24],['28 Jul',27],['4 Aug',48],['11 Aug',33],['18 Aug',25]],25)}
  <div class="stakefoot"><span>Average stake <b>£30</b></span><span>Biggest week <b style="color:var(--neg)">£48 · 1.9u</b></span></div></div>`,
...
const EXPLAINER=`<div class="vidwrap">
 <video class="vid" controls preload="none" playsinline muted
   poster="/video/explainer-poster.jpg" aria-label="How Slippery works, twenty four seconds">
  <source src="/video/explainer.webm" type="video/webm">
  <source src="/video/explainer.mp4" type="video/mp4">
  <p class="note">Your browser cannot play this. <a href="/video/explainer.mp4">Download it instead.</a></p>
 </video>
 <p class="note vidcap">Twenty four seconds, no sound.</p>
</div>`;
...
const snapFrame=(h,crop)=>`<div class="snapframe" style="height:${h}px;--crop:${crop||0}px">`;
const SOCDECK=()=>[
 ['Your groups',`<div class="sbslide">
  ${[['Ultras','U',12,4,'+63.4u'],['Sunday League','SL',7,2,'−4.1u']].map(g=>`
   <div class="card t"><div class="hd" style="margin:0">
    <div style="display:flex;gap:11px;align-items:center"><span class="gpic">${g[1]}</span>
     <div style="text-align:left"><b style="font-size:14.5px">${g[0]}</b>
      <div class="dd" style="font-size:11.5px;color:var(--t3)">${g[3]} of ${g[2]} members</div></div></div>
    <b class="mono" style="color:var(--${g[4].startsWith('−')?'neg':'pos'})">${g[4]}</b></div></div>`).join('')}
  <button class="btn ghost full sm">Discover groups</button></div>`],
 ['Find one to join',`<div class="sbslide">
  ${[['HB Value','HB',38,'+3.4%','Request'],['Irish Racing','IR',21,'+2.8%','Enter code'],['Weekend Multis','WM',54,'−2.1%','Join']].map(g=>`
   <div class="card t"><div class="hd" style="margin:0 0 8px">
    <div style="display:flex;gap:10px;align-items:center"><span class="gpic" style="width:30px;height:30px;font-size:11px;border-radius:9px">${g[1]}</span>
     <div style="text-align:left"><b style="font-size:14px">${g[0]}</b>
      <div class="dd" style="font-size:11px;color:var(--t3)">${g[2]} members · avg ROI ${g[3]}</div></div></div></div>
    <div class="joinrow"><span class="joinwhy">${g[4]==='Join'?'Anyone can join':g[4]==='Enter code'?'A code lets you in':'An admin approves you'}</span>
     <button class="btn ${g[4]==='Join'?'':'ghost'} sm">${g[4]}</button></div></div>`).join('')}</div>`],
 ['Inside a group',`<div class="sbslide">
  <div class="grpbox">
   <div class="grphd"><span class="lbl">Ultras · 12 members</span><span class="lbl">This month</span></div>
   ${[[1,'BlueSlip','+18.2u'],[2,'KerryEdge','+11.7u'],[3,'FiveFolds','+8.4u'],[4,'You','+6.3u']].map(m=>`
    <div class="grprow"><span class="rk">${m[0]}</span><span class="nm">${m[1]}</span>
     <b class="mono" style="color:var(--pos)">${m[2]}</b></div>`).join('')}
   <div class="grpfoot">Everyone in units. Stake size never decides it.</div></div>
  <div class="chal" style="margin-top:10px"><div class="chalhd"><b>First to +20u</b><span class="pill">Ends in 9 days</span></div>
   ${[['BlueSlip','+18.2u',91],['You','+6.3u',32]].map((r,i)=>`
    <div class="chalrow"><span class="rk ${i?'':'gold'}">${i+1}</span><span class="nm">${r[0]}</span>
     <span class="chalbar"><i style="width:${r[2]}%"></i></span><b class="mono">${r[1]}</b></div>`).join('')}</div></div>`],
 ['Someone you follow',`<div class="sbslide">
  <div class="card" style="text-align:center">
   <span class="av" style="width:58px;height:58px;font-size:20px;margin:0 auto 10px">BS</span>
   ...
   <div class="g3" style="margin-top:13px">
    ${[['Units','+18.2u','pos'],['ROI','+9.4%','pos'],['Bets','214','']].map(x=>`
     <div class="stat"><div class="k">${x[0]}</div><div class="v mono"${x[2]?' style="color:var(--pos)"':''}>${x[1]}</div></div>`).join('')}</div>
   <button class="btn ghost full sm" style="margin-top:12px">Following</button></div></div>`],
 ['What others can see',`<div class="sbslide"><div class="grpbox">
  <div class="grphd"><span class="lbl">Privacy</span></div>
  <div style="padding:14px 15px;display:grid;gap:12px">
   ${[['Who can see your figures','Friends only'],['Show money amounts','Off'],['Show unit size','Inside groups'],['Appear in Discover','Off']].map(r=>`
    <div class="row" style="font-size:13px;justify-content:space-between"><span style="color:var(--t3)">${r[0]}</span><b>${r[1]}</b></div>`).join('')}</div>
  <div class="grpfoot">Nothing is public unless you turn it on.</div></div></div>`]];
const IMPDECK=()=>[
 ['Pick where it came from',`<div class="sbslide">
  ${[['Spreadsheet or CSV','Full bets if the columns are there, figures only if not'],
     ...
     ['Type the figures','A date and an amount, nothing else']].map(x=>`
   <button class="setrow"><div><div class="k">${x[0]}</div><div class="dd">${x[1]}</div></div><span class="caret">▾</span></button>`).join('')}</div>`],
 ['It reads every row',`<div class="sbslide">
  <div class="hd" style="margin:0 0 10px"><div><b style="font-size:14px">18 rows read</b>
   <div class="dd" style="font-size:11.5px;color:var(--t3)">Full bets · 2 look like duplicates</div></div><span class="pill">CSV</span></div>
  <div class="hd" style="margin:0 0 6px"><span class="lbl">16 ticked</span><span class="lbl">Toggle all</span></div>
  ${[['Arsenal v Spurs','4 Aug','+£180.00',1],['Leeds v Burnley','7 Aug','−£45.00',1],
     ['Kempton 19:45','9 Aug','+£275.00',1],['Kempton 19:45','9 Aug · duplicate','+£275.00',0]].map(r=>`
   <div class="bet" style="align-items:center"><span class="ic ${r[3]?'w':''}">${r[3]?'✓':''}</span>
    <div class="m"><div class="n">${r[0]}</div><div class="d">${r[1]}</div></div>
    <b class="mono" style="color:var(--${r[2].startsWith('−')?'neg':'pos'})">${r[2]}</b></div>`).join('')}
  <button class="btn full sm" style="margin-top:10px">Import 16 rows</button></div>`]];
const BOTDECK=[
 ...
 ['It settles itself',tgWin('Slippery','private chat','S',
  `<div class="tgb in"><div class="who2">Slippery</div><b>Full time · won</b><br>+£45.00 · +1.80u<br>
   <span>Today: +£${TDY.toFixed(2)} across 3 bets</span><div class="tgtime">21:48</div></div>
   <div class="tgb in"><div class="who2">Slippery</div><b>Full time · lost</b><br>−£25.00 · −1.00u<br>
   <span>2 still open</span><div class="tgtime">21:52</div></div>`)],
 ...
 ['When it cannot read',tgWin('Slippery','private chat','S',
  `<div class="tgb out">📷 blurry.png<div class="tgtime">18:04 ✓✓</div></div>
   <div class="tgb in"><div class="who2">Slippery</div><b>Could not read the odds</b><br>
...
function snapView(fn,tab){const inner=fn();
 return `<div class="snapwrap"><div class="snapinner" data-snap aria-hidden="true" inert>${stripIds(inner)}</div></div>`}
const FILM=[
 ...
 ['Add a bet','The screen you land on',()=>`<div class="sbslide">
  <div class="drop"><div class="dropi">↑</div><b>Screenshot, PDF or CSV</b>
   <span>Drop it here, or tap to choose</span></div>
  <button class="setrow" style="margin-top:10px"><div><div class="k">Type it instead</div><div class="dd">Event, selection, odds, stake</div></div><span class="caret">›</span></button>
  <button class="setrow"><div><div class="k">Forward slips to the bot</div><div class="dd">Telegram, one tap</div></div><span class="pill">Set up</span></button></div>`],
 ['It reads the slip','Every leg, price and stake',()=>`<div class="sbslide">
  <div class="hd" style="margin:0 0 10px"><b style="font-size:14px">4 legs · bet365</b><span class="pill a">Reading</span></div>
  ${[['Under 4 Cards','Total cards','1.91'],['Under 4.5 Shots on Target','Cremonese','2.10'],
     ['Under 11.5 Shots','Cremonese','1.80'],['Juventus to win','Match result','1.44']].map((l,i)=>`
   <div class="bet" style="align-items:center"><span class="ic ${i<3?'w':''}">${i<3?'✓':'·'}</span>
    <div class="m"><div class="n">${l[0]}</div><div class="d">${l[1]}</div></div>
    <b class="mono">${l[2]}</b></div>`).join('')}
  <div class="row" style="margin-top:10px;justify-content:space-between;font-size:12.5px">
   <span>Stake <b>£100.00</b></span><span>Combined <b class="mono">11.42</b></span></div></div>`],
 ['You confirm what it found','Three bets from one screenshot',()=>`<div class="sbslide">
  ${[['Juventus v Cremonese','4 legs · 1.80 · £100 · bet365'],['Arsenal, Leeds, Sinner','3 legs · 8.20 · £40 · Sky Bet'],
     ['Kempton 19:45','Single · 4.50 · £20 · Coral']].map(x=>`
   <div class="card t" style="margin-bottom:9px"><div class="hd" style="margin:0">
    <div style="text-align:left"><b style="font-size:13.5px">${x[0]}</b>
     <div class="dd" style="font-size:11px;color:var(--t3)">${x[1]}</div></div>
    <span class="pill g">✓</span></div></div>`).join('')}
  <button class="btn full sm">Save all three</button></div>`],
 ['Your dashboard updates','Net this month, then the calendar',()=>snapView(()=>`<div class="pane">${netCard()}${calCard()}</div>`,'OVERVIEW')],
...
<div class="lhero"><div class="inner">
 ...
 <div class="herocta"><button class="btn" data-go="su1">Start tracking free</button>
  <button class="herolink" data-go="demo">or see a live demo first</button></div>
...
<div class="lsec c"><h2 class="lh2">Send a bet on Telegram.<br>It is tracked instantly.</h2>
 ...
 <div class="tgbox">
  ...
  <details class="disc botdrop"><summary>
   <span style="display:flex;gap:9px;align-items:center;font-size:14px;font-weight:600">${TG} Preview</span><span class="caret">▾</span></summary>
   <div style="padding-top:8px">
    ...
    <div class="deckbar"><button class="nav" data-dnav="-1" data-deckid="bot" aria-label="Previous">&lsaquo;</button>
     <div class="dots" data-deckdots="bot">${BOTDECK.map((_,i)=>`<i class="${i?'':'on'}"></i>`).join('')}</div>
     <button class="nav" data-dnav="1" data-deckid="bot" aria-label="Next">&rsaquo;</button></div></div></details></div></div>
...
<div class="lsec c payhue" data-sec="price"><h2 class="lh2">Pick how you pay.</h2>
 ...
 <div class="plan pick2"><div class="top"><h3>Annual</h3><span class="badge">Most popular</span></div>
  ...
  <button class="btn full" data-go="su1">Choose annual</button></div></div></div>

<div class="lsec c" data-sec="import"><h2 class="lh2">Switch in seconds.</h2>
 <p class="lsub" style="margin-bottom:22px">Bring your whole history across from wherever it lives now.</p>
 <div class="tgbox aura">
  <ul class="tgflow">
   ${[['Export from your old tracker','CSV or Excel, however it comes out'],['Drop the file in','No column mapping to learn'],['Every row read and checked','Duplicates flagged, nothing overwritten']].map(x=>`
    <li class="tgstep"><span class="tgn"></span><div><b>${x[0]}</b><span>${x[1]}</span></div></li>`).join('')}</ul>
  <div class="deck" data-deck="imp" style="background:transparent;border:0;margin-top:6px"><div class="dtrack">
   ${IMPDECK().map(x=>`<div class="dslide" style="padding:2px 3px 4px"><div class="lbl" style="margin-bottom:9px;text-align:left">${x[0]}</div>${x[1]}</div>`).join('')}</div></div>
  <div class="deckbar"><button class="nav" data-dnav="-1" data-deckid="imp" aria-label="Previous">&lsaquo;</button>
   <div class="dots" data-deckdots="imp">${IMPDECK().map((_,i)=>`<i class="${i?'':'on'}"></i>`).join('')}</div>
   <button class="nav" data-dnav="1" data-deckid="imp" aria-label="Next">&rsaquo;</button></div></div></div>
<div class="lsec c newfeat" data-sec="social"><span class="newtag">New</span>
 <h2 class="lh2">Social.</h2>
 <p class="lsub" style="margin-bottom:20px">Compare units with your friends. Browse groups, start your own, follow people whose record stands up. Nothing is public until you say so.</p>
 <div class="deck aura" data-deck="soc" style="background:transparent;border:0"><div class="dtrack">
  ${SOCDECK().map(x=>`<div class="dslide" style="padding:2px 3px 4px"><div class="lbl" style="margin-bottom:9px;text-align:left">${x[0]}</div>${x[1]}</div>`).join('')}</div></div>
 <div class="deckbar"><button class="nav" data-dnav="-1" data-deckid="soc" aria-label="Previous">&lsaquo;</button>
  <div class="dots" data-deckdots="soc">${SOCDECK().map((_,i)=>`<i class="${i?'':'on'}"></i>`).join('')}</div>
  <button class="nav" data-dnav="1" data-deckid="soc" aria-label="Next">&rsaquo;</button></div></div>
<div class="lsec c" data-sec="themes"><h2 class="lh2">Pick a theme.</h2>
 <p class="lsub" style="margin-bottom:22px">Pick a theme and the whole page changes with it.</p>
...
V.ledger={nav:'dash',tab:'LEDGER',run:1,html:()=>{
 ...
 <div class="card" data-cardid="lrows">
  ...
  <div style="display:flex;justify-content:center;padding-top:13px"><button class="btn ghost sm" data-toast="12 more loaded">Load more</button></div></div>
...
V.person={narrow:1,nav:'soc',back:'social',title:'BlueSlip',html:()=>`<div class="pane">
 <div class="card" style="text-align:center"><span class="av" style="width:58px;height:58px;font-size:20px;margin:0 auto 10px">BS</span>
  ...
  <button class="btn ghost full sm" style="margin-top:12px" data-toast="Disconnected from BlueSlip">Following</button></div>
...
V.social={nav:'soc',tab:'GROUPS',html:()=>`<div class="pane">
 <div class="hd"><span class="lbl">${cur.socTab==='people'?'People':'Your groups'}</span>
  <button class="chip" data-sheet="${cur.socTab==='people'?'tipsters':'creategroup'}">${cur.socTab==='people'?'Find people':'+ New group'}</button></div>
 ${(cur.socTab||'groups')==='people'?`
  <div style="max-width:300px;margin-bottom:12px">${segHTML(['Connected','Following'],'Connected')}</div>
  ${[['BlueSlip','BS','+18.2u','+9.4% ROI'],['KerryEdge','KE','+11.7u','+4.1% ROI'],['FiveFolds','FF','+8.4u','+2.2% ROI']].map(f=>`
   <button class="card t" style="width:100%" data-go="person"><div class="hd" style="margin:0">
    ...
    <b class="mono" style="color:var(--pos)">${f[2]}</b></div></button>`).join('')}`
...
V.groupdetail={narrow:1,nav:'soc',back:'social',title:'Ultras',html:()=>`<div class="pane">
 <div class="card"><div class="hd"><div style="display:flex;gap:11px;align-items:center"><span class="gpic">U</span>
  ...
  <p class="note">Every figure here has a bookmaker slip behind it. Group members see each other's unit size; outside a group only units are shown.</p></div></div>`};
...
V.manual={narrow:1,nav:'imp',html:()=>`<div class="pane"><h2 style="font-size:22px;margin:0 0 10px">Type it in</h2>
 ...
 <label class="flabel">Selection</label><input class="field" id="selIn" autocomplete="off" spellcheck="false" aria-label="Selection" placeholder="Arsenal to win" list="mkList">
...
V.imphist={narrow:1,nav:'imp',back:'import',title:'Import history',html:()=>`<div class="pane">
 ...
 <div class="factlist" style="margin-top:10px"><b style="font-size:13px">What imported history can and cannot do</b>
...
V.su1={bare:1,html:()=>wiz(1,6,'Create your account','Takes about a minute.',`
 <label class="flabel">Email</label><input class="field" type="email" inputmode="email" autocomplete="email" spellcheck="false" aria-label="Email address" placeholder="you@example.com" value="seun4150@gmail.com">
 <label class="flabel">Password</label><input class="field" type="password" autocomplete="new-password" aria-label="Password" placeholder="At least 8 characters" value="Slippery!26">
 <div class="rules">${['8 characters','One capital','One special'].map(r=>`<span>✓ ${r}</span>`).join('')}</div>
 <label class="setrow" style="margin-top:16px"><div><div class="k">I am 18 or over</div><div class="dd">And I agree to the <button class="lnk" data-doc="terms">Terms</button> and <button class="lnk" data-doc="privacypol">Privacy policy</button></div></div><button class="tog" aria-pressed="true" data-tog></button></label>
 <button class="btn full" id="su1go" disabled aria-disabled="true" data-go="su2">Continue</button>
 <p class="note" id="docHint" style="text-align:center;margin-top:7px">Open and read both documents to continue.</p>
 ...
 <button class="btn ghost full" data-toast="Google sign-up"><svg viewBox="0 0 24 24" width="16" height="16" style="margin-right:8px;vertical-align:-3px"><path fill="#4285F4" d="M23 12.3c0-.8-.1-1.6-.2-2.3H12v4.5h6.2a5.3 5.3 0 0 1-2.3 3.5v2.9h3.7c2.2-2 3.4-5 3.4-8.6z"/><path fill="#34A853" d="M12 24c3.1 0 5.7-1 7.6-2.8l-3.7-2.9c-1 .7-2.3 1.1-3.9 1.1-3 0-5.5-2-6.4-4.7H1.7v3C3.6 21.4 7.5 24 12 24z"/><path fill="#FBBC05" d="M5.6 14.7a7.2 7.2 0 0 1 0-4.6v-3H1.7a12 12 0 0 0 0 10.6l3.9-3z"/><path fill="#EA4335" d="M12 4.8c1.7 0 3.2.6 4.4 1.7l3.3-3.3C17.7 1.2 15.1 0 12 0 7.5 0 3.6 2.6 1.7 6.4l3.9 3C6.5 6.8 9 4.8 12 4.8z"/></svg>Sign up with Google</button>
...
V.su3={bare:1,html:()=>wiz(3,6,'Pick a display name','Groups and followers see this. One change a month after that.',`
 <input class="field" autocomplete="off" value="EdgeMargin60" style="margin-top:10px">
 ...
 <details class="disc" style="margin-top:8px"><summary style="border:0"><span style="font-size:13.5px;color:var(--s)">Have a promo or referral code?</span><span class="caret">▾</span></summary>
  <input class="field" autocomplete="off" value="SEUN-8QK4" style="margin-top:0">
...
V.login={bare:1,html:()=>`<div class="pane"><h2 style="font-size:22px;margin:20px 0 5px">Sign in</h2>
 ...
 <label class="flabel">Username or email</label><input class="field" autocomplete="off" value="EdgeMargin60">
...
V.bs_failed={narrow:1,nav:'set',back:'settings',title:'Plan',html:()=>`<div class="pane">
 <div class="card" style="border-color:color-mix(in srgb,var(--neg) 40%,transparent)"><div class="hd"><b style="color:var(--neg)">Payment declined</b></div>
  <p class="note" style="margin:0 0 11px;color:var(--t2)">Everything still works. We try again tomorrow.</p>
...
Object.assign(SH,{
 ...
 bankrolladapt:()=>`<div class="hd"><b style="font-size:16px">Bankroll</b></div>
  ...
  <div class="brline" style="margin-top:4px"><span>Started with <b>£${BR_START.toLocaleString('en-GB')}</b></span>
 ...
 exposure:()=>`<div class="hd"><b style="font-size:16px">Open exposure</b><span class="chip a">£88 at risk</span></div>
  ...
  <div class="brline"><span>Started with <b>£${BR_START.toLocaleString('en-GB')}</b></span>
 ...
 weekly:()=>`<div class="hd"><b style="font-size:16px">Weekly summary</b><span class="pill g">On</span></div>
  ...
  <div class="wkcard">
   ...
   <div class="wknet">+£${WTD.toFixed(2)}</div>
 ...
 challenge:()=>{const g=cur.chalGroup||'All';
  ...
  ${(cur.chalTab||'Running')==='Running'?
   (list.length?list.map(c=>`<div class="chal">
    <div class="chalhd"><div><b>${c[1]}</b>${g==='All'?`<div class="dd" style="font-size:11px;color:var(--t4)">${c[0]}</div>`:''}</div>
 ...
 bankroll:()=>`<div class="hd"><b style="font-size:16px">Bankroll</b></div>
  ...
  <div class="setrow" style="margin-top:12px;border-top:1px solid var(--line);padding-top:14px">
   <div><div class="k">Adaptive bankroll</div><div class="dd">Your bankroll moves with settled results, so exposure reads against what you actually hold</div></div>
   <button class="tog" data-adapt aria-pressed="${cur.adaptBr!==false}"></button></div>
  <div class="brline"><span>Started with <b>£${BR_START.toLocaleString('en-GB')}</b></span>
   <span>Now <b>${money(bankroll()).replace('+','')}</b></span></div>
  <p class="note" style="margin-top:8px">${cur.adaptBr!==false?'Exposure is measured against '+money(bankroll()).replace('+','')+'.':'Exposure is measured against your starting balance only.'}</p>
  <button class="btn ghost full sm" style="margin-top:11px" data-resetbr>Reset to starting balance</button>`,
 ...
 legalDoc:(t)=>{const isT=(t==='terms'),D=isT?TERMSDOC:PRIVDOC;
  ...
  <div class="ackbar"><p class="note" style="margin:0 0 9px">Read to the end, then confirm.</p>
   <button class="btn full sm" data-ack="${isT?'terms':'privacypol'}">I acknowledge</button></div>
...
const NAVI=a=>`<nav class="navbar" aria-label="Sections">
 ...
 <div class="navextra">
  ...
  <button class="navsub" data-sheet="bankroll">Bankroll<span class="nbadge">${money(bankroll()).replace('+','')}</span></button>
...
const SOCTABS=()=>`<div class="tabs">${[['groups','GROUPS'],['people','PEOPLE']].map(x=>`<button aria-current="${(cur.socTab||'groups')===x[0]}" data-soctab="${x[0]}">${x[1]}</button>`).join('')}</nav>`;
...
const BAR=v=>v.back?`<div class="topback"><button data-go="${v.back}" style="font-size:20px">‹</button>${v.title}</div>`
 :`<div class="appbar">${BRAND}
   <div class="barright">
    ${v.run?`<button class="runpill" data-sheet="exposure" aria-label="Open exposure, £${BR_OPEN} at risk, ${exposurePct().toFixed(1)} percent of bankroll"><span class="dot"></span><span class="mono">£${BR_OPEN}</span><span class="pct">${exposurePct().toFixed(1)}%</span></button>`:''}
...
function go(id){const v=V[id];if(!v)return;
 ...
 ph.innerHTML=`<main class="body" id="main">${heading}${v.bare?'':BAR(v)}${v.tab?(v.nav==='soc'?SOCTABS():TABS(v.tab)):''}${v.html()}</main>${v.bare?'':NAVI(v.nav)}`;
...
function checkDocs(){const t=ph.querySelector('[data-doc=terms]'),p=ph.querySelector('[data-doc=privacypol]'),b=document.getElementById('su1go');
 if(!b)return;const ok=t&&p&&t.dataset.scrolled&&p.dataset.scrolled;
 b.toggleAttribute('disabled',!ok);b.setAttribute('aria-disabled',!ok);
 const h=ph.querySelector('#docHint');if(h)h.hidden=!!ok}
...
doc.addEventListener('click',e=>{
 ...
 const ct=e.target.closest('[data-cal-toggle]');if(ct){
  ...
  if(open){cur.SIZE.cal='c';buildCal(c,'week');w.style.maxHeight='150px';ct.textContent='Expand month';ti.textContent='This week';}
  else{cur.SIZE.cal='f';buildCal(c,'month');w.style.maxHeight='';ct.textContent='Collapse';ti.textContent='August 2026';}
  if(ti)ti.textContent=open?'This week':'August 2026';
  card.className='card sz-'+cur.SIZE.cal;
 ...
 const th=e.target.closest('[data-theme]');if(th){const t=th.dataset.theme;if(t===cur.theme)return;
  cur.theme=t;ph.dataset.t=t;document.body.dataset.t=t;
 ...
 const doc=e.target.closest('[data-doc]');if(doc){doc.dataset.read=1;sheet(doc.dataset.doc);
  setTimeout(()=>{const sh=ph.querySelector('.sheet');if(!sh)return;
   sh.addEventListener('scroll',()=>{if(sh.scrollTop+sh.clientHeight>=sh.scrollHeight-40)doc.dataset.scrolled=1;checkDocs()});
   if(sh.scrollHeight<=sh.clientHeight+10){doc.dataset.scrolled=1;checkDocs()}},420);
  checkDocs();return}
 const su=e.target.closest('[data-setunit]');if(su){const v=su.dataset.setunit;
  ...
  const eg=document.getElementById('unitEg');if(eg)eg.innerHTML=unitExample(cur.unit);return}
...
function calCell(d,today){const v=DAYVALS[d]||0,fut=d>today,none=!v&&!fut;
 return `<${v?'button':'div'} class="c ${v>0?'p':v<0?'n':''} ${none?'none':''} ${fut?'fut':''} ${d===today?'today':''}" ${v?`data-day="${d}"`:''}>
  ${v?`<span>${cur.calUnits?(v>0?'+':'')+(v/cur.unit).toFixed(1)+'u':(v>0?'+':'')+v}</span>`:''}${cur.calDates?`<span class="dn">${d}</span>`:''}</${v?'button':'div'}>`}
...
function measureNav(){
 ...
 const set=()=>{const h=Math.ceil(bar.getBoundingClientRect().height);if(h)ph.style.setProperty('--navh',h+'px')};
 set();
...
function markNotBuilt(root){
 root.querySelectorAll('[data-toast]').forEach(el=>{
  ...
  el.toggleAttribute('data-notbuilt',!ready);
...
function boot(){
 ...
 document.querySelectorAll('[data-cal]:not([data-d])').forEach(el=>{el.setAttribute('data-d',1);
  const narrow=document.body.classList.contains('vp-phone')||innerWidth<=640;
  const md=el.dataset.cal==='auto'?(narrow?'week':'month'):el.dataset.cal;buildCal(el,md);
...
function runSlip(){const host=document.getElementById('slipHost');if(!host)return;let idx=0;slipVis=true;
 ...
 (function cycle(){
  ...
  host.innerHTML=`<div class="slipframe">${slipHTML(idx)}</div>`;
