...
const APPLEBADGE=`<span class="badgelink" aria-label="Download on the App Store">
<svg viewBox="0 0 120 40" role="img"><rect x=".5" y=".5" width="119" height="39" rx="7" fill="#000" stroke="#A6A6A6"/>
...
const PLAYBADGE=`<span class="badgelink" aria-label="Get it on Google Play">
<svg viewBox="0 0 152 40" role="img"><rect x=".5" y=".5" width="151" height="39" rx="7" fill="#000" stroke="#A6A6A6"/>
...
const THEMES=[['periwinkle','Periwinkle','Indigo on deep navy. The default.','#0A0F1E','#6D86DB','#F2F5FA'],
 ['ink','Ink','Near black, violet cast. The darkest.','#050508','#8B84C4','#F3F1FA'],
 ['graphite','Graphite','No colour but the figures.','#121417','#8A99AE','#F1F3F6'],
 ['slate','Slate','Steel blue-grey. The lightest dark.','#161A21','#7E93B5','#EEF2F7'],
 ['tide','Tide','Pastel teal. The coolest.','#0D1E24','#8FCBD4','#EFF8FA'],
 ['bronze','Bronze','Warm charcoal. The only warm dark.','#15120F','#C9915E','#F7F1EA'],
 ['light','Light','Soft grey-white. Easy in daylight.','#EFF1F5','#5468BE','#1B2233'],
 ['linen','Linen','Warm paper. The gentlest.','#F5F1E8','#8A6A44','#2A2318']];
...
V.ledger={nav:'dash',tab:'LEDGER',run:1,html:()=>{
 ...
 const facets=[['All',bets.length,'',1],['Won',c.w,'w',0],['Lost',c.l,'l',0],['Cashed',2,'a',0],['Void',1,'',0]];
 return `<div class="pane">
 ...
 <div class="card"><div class="lbl" style="margin-bottom:9px">Appearance</div>
  <div class="themes">${THEMES.map(t=>`<button class="tcard" aria-current="${t[0]===cur.theme}" data-theme="${t[0]}">
   <div class="tprev" style="background:${t[3]}"><div class="l1" style="background:${t[5]}22"></div><div class="l2" style="background:${t[5]}14"></div>
    <div class="bar" style="background:${t[4]}"></div><span class="up" style="color:${t[0]==='light'?'#12925A':'#86EFAC'}">+£48</span>
    <span class="dnn" style="color:${t[0]==='light'?'#D0364B':'#FCA5A5'}">−£12</span></div>
...
/* ═══ router ═══ */
const NAVI=a=>`<div class="navbar">
 ${[['dash','overview','dsh','Dashboard'],['imp','import','impi','Add a bet'],['soc','social','soci','Social'],['set','settings','seti','Settings']]
 .map(x=>`<button aria-current="${a===x[0]}" data-go="${x[1]}"><svg viewBox="0 0 24 24"><use href="#${x[2]}"/></svg>${x[3]}</button>`).join('')}</div>`;
...
function go(id){const v=V[id];if(!v)return;
 cur.view=id;
 ph.innerHTML=`<div class="body">${v.bare?'':BAR(v)}${v.tab?TABS(v.tab):''}${v.html()}</div>${v.bare?'':NAVI(v.nav)}`;
 ph.toggleAttribute('data-narrow',!!v.narrow);
 ph.toggleAttribute('data-cols',!!v.cols);
 const bd=ph.querySelector('.body');
 [...bd.children].forEach((c,i)=>{c.style.animationDelay=(i*55)+'ms'});
...
function sheet(k){if(!SH[k])return;
 ...
 ph.insertAdjacentHTML('beforeend',`<div class="scrim" data-close></div><div class="sheet"><div class="grab"></div>${SH[k]()}</div>`);
...
function toast(m,undo){ph.querySelectorAll('.toast').forEach(t=>t.remove());
 ph.insertAdjacentHTML('beforeend',`<div class="toast">${undo?`<span style="display:flex;align-items:center;gap:10px"><span style="flex:1">${m}</span><button data-undo style="color:var(--s);font-weight:700;font-size:13px;white-space:nowrap">Undo</button></span>`:m}</div>`);
