...
/* Guarded: a bankroll of zero is a division by zero, and the answer people
   need there is nothing at risk, not Infinity. */
const exposurePct=()=>{const b=bankroll();return b>0?(BR_OPEN/b*100):0};
...
V.ledger={nav:'dash',tab:'LEDGER',run:1,html:()=>{
 ...
 <div class="card" data-cardid="lsum"><div class="lsumgrid">
  ${[['Staked','£'+staked.toFixed(0),0],['Returned','£'+ret.toFixed(0),0],['Net',money(net),1],['ROI',roiOf({net,to:staked,void:0}),1]].map(x=>`
...
const BAR=v=>v.back?`<div class="topback"><button data-go="${v.back}" style="font-size:20px">‹</button>${v.title}</div>`
 :`<div class="appbar">${BRAND}
   <div class="barright">
    ${v.run&&BR_OPEN>0?`<button class="runpill" data-sheet="exposure" aria-label="Open exposure, £${BR_OPEN} at risk, ${exposurePct().toFixed(1)} percent of bankroll"><span class="dot"></span><span class="mono">£${BR_OPEN}</span><span class="pct">${exposurePct().toFixed(1)}%</span></button>`:''}
