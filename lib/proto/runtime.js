...
let bets=[
 ...
 /* 55 · The bet the six outcomes could not express. £20 each way at 6.00,
    1/5 odds, places 1-3, finished 3rd of 12. The old model called it LOST at
    −£20.00; it returned £16.00, so the answer is −£4.00 and the word is
    PLACED. Two child parts, one row. */
 ['p','York 16:10','Selection 9','Win',6.00,20,16,'−£4.00','−0.16u','Sky Bet','Own picks','Screenshot','16 Aug 2026, 16:10',0,null,
  {ew:{frac:'1/5',places:3,win:{stake:10,price:6.00,out:'l',ret:0},place:{stake:10,price:2.00,out:'w',ret:20}}}],
...
Object.assign(SH,{
 ...
 bankroll:()=>{
  ...
  <input class="field mono" id="brstart" inputmode="decimal" value="${(st/1).toFixed(2)}">
  <div class="card t" style="margin-top:12px">
   ...
   <div class="setrow" style="padding:8px 0"><div class="k" style="font-size:13px">Growth</div>
    <b class="mono" style="font-size:13px;color:var(--${(g||0)<0?'neg':'pos'})">${g==null?'—':(g>0?'+':'')+g.toFixed(0)+'%'}</b></div></div>
 ...
 adjust:()=>`<div class="hd"><b style="font-size:16px">Money in or out</b></div>
  ...
  <input class="field mono" id="adjamt" inputmode="decimal" placeholder="0.00">
  <div style="margin:11px 0">${segHTML(['Deposit','Withdrawal'],cur.adjKind||'Deposit','adjkind')}</div>
