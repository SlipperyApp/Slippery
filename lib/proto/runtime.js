...
const BRAND=`<button class="brand" data-go="landing" data-home>${WORDMARK_GET()}</button>`;
const BRANDMARK=`<button class="brand" data-go="landing" data-home>${MARK}${WORDMARK_GET()}</button>`;
...
const amount=n=>'£'+Math.abs(Number(n)||0).toLocaleString('en-GB',{minimumFractionDigits:2,maximumFractionDigits:2});
...
V.import={narrow:1,nav:'imp',html:()=>`<div class="pane">
 ...
 <button class="card" style="width:100%;border-style:dashed;border-color:var(--p);text-align:center;padding:26px 16px" data-pick>
  <div style="margin-bottom:7px"><svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true"><use href="#impi"/></svg></div><b style="font-size:14px;display:block">Screenshot, PDF or CSV</b>
  <p class="note" style="margin-top:4px">Drop it here, or tap to choose</p></button>
 <input type="file" data-slipinput hidden multiple accept="image/*,application/pdf,.csv,.heic,.heif">
...
V.review={narrow:1,nav:'imp',html:()=>{
 ...
 if(!bets.length)return `<div class="pane" style="padding-top:40px;text-align:center">
  <h2 style="font-size:20px;margin:0 0 8px">Nothing to check</h2>
  <p class="note" style="margin:0 0 18px">No slip has been read yet.</p>
...
const SOCTABS=()=>`<div class="tabs">${[['groups','GROUPS'],['people','PEOPLE']].map(x=>`<button aria-current="${(cur.socTab||'groups')===x[0]}" data-soctab="${x[0]}">${x[1]}</button>`).join('')}</div>`;
...
async function saveRead(button){
  ...
  if(saved){
    ...
    if(!cur.readBets.length){cur.readName='';hydrateFrom('/api/bets');go('overview')}
    else repaint();
  }
...
/* Re-read the ledger after a write, so the dashboard is the server's answer
   rather than this session's guess at it. */
function hydrateFrom(path){
  fetch(path,{credentials:'same-origin'}).then(r=>r.ok?r.json():null).then(j=>{
    if(!j)return;
    Object.assign(cur,j);
    repaint();
  }).catch(()=>{});
