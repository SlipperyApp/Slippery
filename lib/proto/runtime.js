...
const CARDS=[['net','Net and target'],['cal','Calendar'],['recent','Recent bets'],['alltime','All time'],
 ['book','By bookmaker'],['market','By market'],['tips','By tipster'],['sport','By sport'],['curve','Profit over time'],['months','Month by month'],['stake','Staking discipline'],['running','Running now'],['oddsband','By odds band'],['clv','Closing line value']];
...
const DEFAULT_ABOVE=['net','running','cal','recent','alltime','curve','stake','oddsband','book','market','tips','sport','clv'];
...
const DEFAULT_ORDER=['net','running','cal','recent','alltime','curve','stake','oddsband',
 'book','market','tips','sport','clv'];
...
const cur={netOrder:null,chalGroup:'All',chalTab:'Running',shareKinds:['Summary'],netShow:['bets','units'],calUnits:false,promo:'SEUN-8QK4',unit:25,oddsFmt:'Decimal',showIn:'Currency',view:'landing',theme:'carbon',per:'M',calDates:true,weekStart:1,demoSeen:false,signedIn:false,
 ...
 SIZE:{net:'w',cal:'f',curve:'w',recent:'w',alltime:'c',clv:'c',months:'w',running:'s',
...
function szCard(k){
 ...
 if(st==='waiting')return waitingCard(k,title);
...
function moduleCount(id){
 ...
 if(id==='clv')return 0;              /* nothing is priced yet */
 ...
 return `<div class="card run" data-cardid="running"><div class="hd"><b>Running now</b>
  ...
  ${showBar?`<div class="expobar">
   <div class="ebrow"><span>At risk</span>
    <span class="ebfig">£${risk.toFixed(2)}<span class="ebto"> to £${ret.toFixed(2)}</span></span></div>
   ...
   <div class="ebrow" style="margin-top:7px"><span>${(risk/bankroll()*100).toFixed(1)}% of bankroll</span>
    <span>Profit if all land <b style="color:var(--pos)">+£${(ret-risk).toFixed(2)}</b></span></div>
  </div>`:''}
  ${sorted.map(o=>{const c=bookColour(o.b),ex=EXCHANGES.has(o.b);
   return `<div class="orow">
    ...
    <span class="oval"><span class="ostake">£${o.s.toFixed(2)}</span>
     ${showRet?`<span class="oret">to £${o.ret.toFixed(2)}</span>`:''}</span></div>`}).join('')}
...
function oddsBandCard(){
 ...
 let out=`<line x1="${zero}" y1="2" x2="${zero}" y2="${rows.length*RH+2}" stroke="var(--line)"/>
  <text x="${zero}" y="${H-3}" text-anchor="middle" font-size="10" fill="var(--t4)">£0</text>`;
 rows.forEach(([n,v,c],i)=>{
  ...
  out+=`<text x="0" y="${y+4}" font-size="12" fill="${thin?'var(--t4)':'var(--t2)'}">${esc(n)}</text>
   ...
   <text x="${W}" y="${y+1}" text-anchor="end" font-size="12.5" font-weight="700"
     fill="${thin?'var(--t4)':(v>0?'var(--pos)':'var(--neg)')}">${v>0?'+':'−'}£${Math.abs(v)}</text>
   <text x="${W}" y="${y+15}" text-anchor="end" font-size="10" fill="var(--t4)">${
...
function perfCard(){
 ...
 /* Eight bars at 390px cannot be read, so a phone opens on the curve. */
 const shape=modv('perf','shape',innerWidth<760?'curve':'both');
 const span=modv('perf','span',8);
 const S=perfSeries(basis,span);
 const W=720,showB=shape!=='curve',showC=shape!=='bars';
 ...
 if(showC){
  ...
  out+=`<line x1="${pL-8}" y1="${Y(0)}" x2="${W}" y2="${Y(0)}" stroke="var(--line)" stroke-dasharray="3 3"/>
   <text x="${pL-12}" y="${Y(0)+4}" text-anchor="end" font-size="10" fill="var(--t4)">${perfFmt(0,basis)}</text>
   <text x="${pL-12}" y="${Y(hi)+4}" text-anchor="end" font-size="10" fill="var(--t4)">${perfFmt(hi,basis)}</text>
 ...
 if(showB){
  ...
  for(let g=st;g<=up;g+=st)out+=`<line x1="${pL-8}" y1="${Yb(g)}" x2="${W}" y2="${Yb(g)}" stroke="var(--line)" stroke-dasharray="3 4"/>
   <text x="${pL-12}" y="${Yb(g)+3.5}" text-anchor="end" font-size="9.5" fill="var(--t4)">${perfFmt(g,basis)}</text>`;
  ...
  out+=`<line x1="${pL-8}" y1="${zero}" x2="${W}" y2="${zero}" stroke="var(--t3)" stroke-width="1.2"/>
   <text x="${pL-12}" y="${zero+3.5}" text-anchor="end" font-size="9.5" fill="var(--t4)">${perfFmt(0,basis)}</text>`;
 ...
 S.forEach((d,i)=>{out+=`<text x="${X(i)}" y="${H-4}" text-anchor="middle" font-size="10" fill="var(--t4)">${d.m}</text>`});
 ...
 return `<div class="dots${open?' open':''}" data-modmenu="${id}">
  ...
  <div class="menu" role="menu">
   ${groups.map(g=>{
    ...
    if(g.type==='check')return `<div class="mi" role="menuitemcheckbox"
      data-modtog="${id}:${g.key}" aria-checked="${
       cur.mod[id]&&cur.mod[id][g.key]!==undefined?cur.mod[id][g.key]:g.value}">${esc(g.label)}</div>`;
    if(g.type==='rule')return '<hr>';
    return `<div class="mi" role="menuitem" data-modhide="${id}">${esc(g.label)}</div>`;
   }).join('')}
   <hr><div class="mi" role="menuitem" data-modhide="${id}">Hide this module</div>
  </div></div>`}
...
function stakeChart(rows,unit){
 ...
 return `<div class="stakeplot">
  <div class="stakeaxis"><span class="sunit" style="left:${mid}%">£${unit} unit</span>
  ...
  ${rows.map(r=>{
   ...
   return `<div class="stakerow">
    ...
    <b class="sval ${inBand?'':tone}">${inBand?'on plan':(d>0?'+':'−')+'£'+Math.abs(d).toFixed(0)}</b>
   </div>`}).join('')}
 </div>
 ...
 <div class="stakefoot"><span>On plan <b>${onPlan} of ${rows.length} weeks</b></span>
  <span>Average <b>£${Math.round(rows.reduce((t,r)=>t+r[1],0)/rows.length)}</b> against a <b>£${unit}</b> unit</span></div>`}
...
const CARDFN={net:netCard,cal:calCard,recent:recentCard,
 ...
 clv:()=>{
  const bins=[2,3,6,9,13,16,14,10,6,3,2],W=470,H=62,bw=W/bins.length,mx=Math.max(...bins);
  const bars=bins.map((v,i)=>`<rect x="${(i*bw+2).toFixed(1)}" y="${(44-v/mx*40).toFixed(1)}"
    width="${(bw-4).toFixed(1)}" height="${(v/mx*40).toFixed(1)}" rx="2.5" fill="var(--elev)"/>`).join('');
  return `<div class="card soon" data-cardid="clv"><div class="hd"><b>Closing line value</b>
   <span class="chip soon">Coming soon</span></div>
   <div class="soonbody">
    <div class="big" style="color:var(--t3)">+0.0%</div>
    <div class="chartwrap"><svg viewBox="0 0 ${W} ${H}" width="100%" aria-hidden="true">${bars}
     <line x1="${W/2}" y1="0" x2="${W/2}" y2="48" stroke="var(--line)" stroke-dasharray="2 3"/>
     <text x="2" y="60" font-size="10" fill="var(--t4)">distribution</text>
     <text x="${W}" y="60" text-anchor="end" font-size="10" fill="var(--t4)">beat the close</text></svg></div>
    <p class="note" style="margin:10px 0 0">CLV predicts long-run profit better than win rate.
     It arrives once imported bets can be told apart from priced ones.</p></div></div>`},
