...
export function DemoNote({ handle }: { handle: string }) {
  ...
  return (
    <div className="banner banner--accent" style={{ marginBottom: 'var(--s4)' }}>
      ...
      <span className="grow">
        You are looking at the example account, <span className="mono">@{handle}</span>. Every figure
        here is folded by the same code your own ledger would use.{' '}
        <Link href="/signup">Start your own</Link>.
      </span>
